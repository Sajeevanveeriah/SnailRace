...
function RunningOrder({
  ...
  return (
    <aside
      className={`race-standings ${shown.length > 8 ? 'race-standings-dense' : ''}`}
      aria-label={`Running order for ${shown.length} runners`}
    >
      <h3>
        STANDINGS <span>{shown.length} RUNNERS</span>
      </h3>
      <ol
        className="tv-order"
        aria-live="off"
        tabIndex={0}
        aria-label="Runner positions"
      >
        {shown.map((row) => (
